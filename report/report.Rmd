...
# find blank col
blank_col <- names(network)[1]

# set first column as rownames
rownames(network) <- network$blank_col

# drop name column
network$blank_col <- NULL
...
bm <- data.matrix(boundaries[2:ncol(boundaries)])
row.names(bm) <- boundaries$name

# https://cran.r-project.org/web/packages/philentropy/vignettes/Distances.html

# calculate jaccard distance
d <- distance(bm, method = "jaccard",)
...
sim <- d[respondent$name,][2:length(boundaries$name)]

# calculate the average similairty
avg_sim <- mean(sim)
```

***Your network is `r percent(avg_sim)` diverse.***
